...
// Use writable directory in serverless / cloud environment (e.g. Vercel)
const isServerless = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME;
const dbPath = isServerless
  ? path.join(os.tmpdir(), 'portfolio.db')
...
try {
  db = new sqlite3.Database(dbPath, (err) => {
    if (err) {
      console.warn('SQLite file warning, using memory fallback:', err.message);
      db = new sqlite3.Database(':memory:');
    }
  });
} catch (e) {
  console.warn('SQLite init catch, using memory DB:', e.message);
  db = new sqlite3.Database(':memory:');
}
